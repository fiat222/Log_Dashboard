...
# Check write permission for the backup directory.
...
# ClickHouse creates backup files inside the mounted backup directory.
...
if echo "$CH_RESP" | grep -qi "Exception\|Error\|failed"; then
    ...
    # Find the directory created by ClickHouse and avoid nested path issues.
